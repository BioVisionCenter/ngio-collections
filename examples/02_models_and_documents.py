...
def build_tree() -> ngc.CollectionNode:
    ...
    return ngc.CollectionNode(id="experiment", name="My Experiment", nodes=[image])


def main() -> None:
    # --- Structural rules are enforced at construction ----------------------
    try:
        ngc.CollectionNode(id="c", name="c")  # neither `nodes` nor `path`
    ...
    print("labels set:", ngc.LabelsAttribute in image.attrs)
    ...
    reparsed = ngc.parse_metadata_document(payload, url="memory://collection.json")
