...
async def db_engines():
    ...
    from ngio_collections.io.store._postgres._models import Base

    ECHO_DB = False  # Set to True to debug SQL statements
    engine_sync = create_engine(SQLALCHEMY_DB_URL, echo=ECHO_DB)
    engine_async = create_async_engine(SQLALCHEMY_DB_URL, echo=ECHO_DB)

    # We only need to create tables through one engine
    metadata = Base.metadata
    metadata.create_all(engine_sync)

    yield dict(engine_sync=engine_sync, engine_async=engine_async)

    # We only need to drop tables through one engine
...
@pytest.fixture(scope="function")
async def pgstore(db_engines):
    return PostgresStore(dataset_id=DATASET_ID, orm_model=Collection, **db_engines)


@pytest.mark.xfail(reason="This test breaks the unique-url constraint.", strict=True)
async def test_create_open_edit_save_roundtrip(pgstore) -> None:
    store = pgstore
...
async def test_compose_via_reference_then_open_and_inline(pgstore) -> None:
    store = pgstore
    # child document
...
async def test_ref_stub_matches_create_stub(pgstore) -> None:
    store = pgstore
...
async def test_minted_stubs_carry_id_and_are_findable_after_reopen(pgstore) -> None:
    store = pgstore
    image = new_node("multiscale", id="image", name="Image")
    stub = await aio.create(f"{DATA}/image.zarr", image, store)
...
async def test_id_less_doc_root_stub_opens_and_resolves(pgstore) -> None:
    store = pgstore
...
async def test_children_literal_mixes_stub_and_embedded(pgstore) -> None:
    store = pgstore
    image = new_node("multiscale", id="image", attributes={"role": "target"})
    stub = await aio.create(f"{DATA}/image.zarr", image, store)
...
async def test_save_inlined_snapshots_to_one_document(pgstore) -> None:
    store = pgstore
...
async def test_subtree_of_inlined_node_is_fully_detached(pgstore) -> None:
    store = pgstore
    image = new_node("multiscale", id="image", attributes={"role": "x"})
    stub = await aio.create(f"{DATA}/image.zarr", image, store)
...
async def test_delete_removes_document(pgstore) -> None:
    store = pgstore
...
@pytest.mark.xfail(reason="This test breaks the unique-url constraint.", strict=True)
async def test_externalize_splits_node_into_own_document(pgstore) -> None:
    store = pgstore
    root = new_node(
        "collection",
        id="root",
...
@pytest.mark.xfail(reason="This test breaks the unique-url constraint.", strict=True)
async def test_externalize_rejects_invalid_targets(pgstore) -> None:
...
async def test_open_ref_resolves_subtree_across_documents(pgstore) -> None:
    store = pgstore
    image = new_node("multiscale", id="image", attributes={"role": "target"})
    stub = await aio.create(f"{DATA}/image.zarr", image, store)
    parent = new_node("collection", id="root").add_ref(stub)
...
async def test_open_inlined_ref_resolves_nested_references(pgstore) -> None:
    store = pgstore
...
async def test_open_ref_missing_id_raises_lookup_error(pgstore) -> None:
    store = pgstore
    image = new_node("multiscale", id="image")
...
async def test_open_ref_without_path_raises(pgstore) -> None:
    from ngio_collections.models._config import NodeStateError
...
async def test_create_rejects_a_tree_built_with_origin_url(pgstore) -> None:
    from ngio_collections.models._config import NodeStateError
...
async def test_create_refuses_existing_without_overwrite(pgstore) -> None:
    from ngio_collections.models._config import NodeStateError
